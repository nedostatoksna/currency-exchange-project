...
const renderLatestApplications = function() {
	...
	for (let i = 0; i < latestApplicationsFromStorage.length; i++) {
		// создаем все элементы карточек 
		let appArticle = document.createElement("article");
		appArticle.classList.add("application-card");
		if (i === 0) {
			appArticle.classList.add("active-card");
		}
		appArticle.dataset.id = `${i + 1}`;

		let appDate = document.createElement("h2");
		appDate.classList.add("transaction-date");
		appDate.textContent = latestApplicationsFromStorage[i].dateString;

		let appDivContent = document.createElement("div");
		appDivContent.classList.add("application-card-content");

		let appGiveDetailsDiv = document.createElement("div");
		appGiveDetailsDiv.classList.add("transaction-details-wrapper");

		let appGiveTransAmount = document.createElement("p");
		appGiveTransAmount.classList.add("transaction-details");
		appGiveTransAmount.classList.add("give-transaction-details");
		appGiveTransAmount.textContent = latestApplicationsFromStorage[i].giveAmount;

		let appGiveTransCurrency = document.createElement("span");
		appGiveTransCurrency.classList.add("transaction-currency");
		appGiveTransCurrency.classList.add("give-card-curr");
		appGiveTransCurrency.textContent = latestApplicationsFromStorage[i].giveCurrency;

		let appIconWrapper = document.createElement("div");
		appIconWrapper.classList.add("transaction-icon");

		let appReceiveDetailsDiv = document.createElement("div");
		appReceiveDetailsDiv.classList.add("transaction-details-wrapper");

		let appReceiveTransAmount = document.createElement("p");
		appReceiveTransAmount.classList.add("transaction-details");
		appReceiveTransAmount.classList.add("receive-transaction-details");
		appReceiveTransAmount.textContent = latestApplicationsFromStorage[i].receiveAmount;

		let appReceiveTransCurrency = document.createElement("span");
		appReceiveTransCurrency.classList.add("transaction-currency");
		appReceiveTransCurrency.classList.add("receive-card-curr");
		appReceiveTransCurrency.textContent = latestApplicationsFromStorage[i].receiveCurrency;

		// создаем кнопки для мобильной версии
		let sliderLabel = document.createElement("label");
		sliderLabel.classList.add("slide-label");
		sliderLabel.setAttribute("for", `${i + 1}`);

		let sliderInput = document.createElement("input");
		sliderInput.classList.add("slider-radio");
		...
		if (i === 0) {
			sliderInput.setAttribute("checked", "true");
		}

		let sliderSpan = document.createElement("span");
		sliderSpan.classList.add("slide");

		// вкладываем элементы кнопок друг в друга
		sliderLabel.appendChild(sliderInput);
		sliderLabel.appendChild(sliderSpan);
	
		// вкладываем элементы карточек друг в друга
...
//вспомогательная функция для рендера latest applications
